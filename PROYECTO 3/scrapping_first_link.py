...
import spacy
def get_url_2 (url, diccionario,contador,posicion,cont2,cont3):
                    ...
                    if todos_hiper[i][1].find(" ")>=0:
                        if todos_hiper[i][1].find("Categoría:")==-1:
                            if todos_hiper[i][1].find("(")==-1:
                                if todos_hiper[i][1].find(":")==-1:
                                    if todos_hiper[i][1].find("Existe un")==-1:
                                        for ent in doc.ents:
                                            if ent.label_ == "PER":
...
while True:
    ...
    if url == '':
        ...
        get_url_2(url,diccionario,0,0,0,cont3)
        cont3 = 0
        for j in range(10):
            for i in range(len(diccionario[list(diccionario.keys())[j]])):
                contador+=1
                get_url_2(list(diccionario.values())[j][0][0],diccionario,contador,j,2,cont3)
            contador=-1
        visited = []
